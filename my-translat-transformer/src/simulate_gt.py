...
def simulate_gt( test_idx, test_set):
    ...
    op_traj_dict['gt_actions'] = gt_actions.cpu()*2*np.pi
...
def main(dataset_type, unseen_dataset_path, dataset_path, context_len, max_test_samples, include_obs_diff):
    random_split= True
    ...
    if dataset_type == "DOLS":
        ...
        test_set = create_action_dataset_v2(us_train_traj_set, 
                                us_test_idx_set,
                                context_len, 
                                norm_params_4_val = src_stats
                                            )
    ...
    for test_sample_idx in range(max_test_samples):
        ...
        results, test_env = simulate_gt(test_sample_idx, test_set)
...
if __name__ == "__main__":
    ...
    for idx, unseen_dataset_path in enumerate(unseen_dataset_path_list):
        ...
        visualize_output(val_set_txy_preds, 
                            ...
                            plot_flow=True,
                            wandb_suffix="val",
                            model_name=f"gt_testset_{files[idx]}_",
                            ) 