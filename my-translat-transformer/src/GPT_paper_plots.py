...
class paper_plots:
    ...
    def plot_obstacle(self, ax, obs_token=None, xyw=None):
        if obs_token != None:
            x_ll, y_ll, w = self.f(obs_token[0:3])
            # y_ll2, x_ll2, w2 = self.f(obs_token[3:])
        elif xyw != None:
            x_ll, y_ll, w = xyw
        else:
        ...
        ax.add_patch(rect)
        # rect2 = patches.Rectangle((x_ll2, y_ll2), w2, w2, linewidth=1, edgecolor='k', facecolor='grey')
        # ax.add_patch(rect2)
        return
    ...
    def plot_val_ip_op(self, traj_dataset,
        ...
        self.setup_ax(ax)       
        im = self.plot_vel_field(ax,t=vmax,r=199)
        self.plot_obstacle(ax, xyw=(49,57,5))
        # self.plot_obstacle(ax, xyw=(27,26,5))
        # traj_dataset=random.shuffle(traj_dataset)
