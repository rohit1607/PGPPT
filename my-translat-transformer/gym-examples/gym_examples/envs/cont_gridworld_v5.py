...
class ContGridWorld_v5(gym.Env):
    ...
    def setup(self, cfg, params2, rzn=0, add_trans_noise=False):
        ...
        self.if_scale_velocity = self.params["scale_velocity"]
        self.space_scale = self.params['space_scale']
        self.ylim, self.xlim = self.params2["grid_dims"]
    ...
    def sanity_checks(self):
        for t in range(self.nT):
            assert(self.is_hitting_obs([t, self.target_pos[0], self.target_pos[1]])==False)
   
    def convert_txy_to_tij(self, txy):
        t, x, y = txy
        t = int(t)
        j = int(np.floor(x))
        i = int(self.ylim - np.floor(y) - 1)
        return t,i,j
    ...
    def is_hitting_obs(self, state=None):
        if state == None:
            state= self.state.copy()
    
        t, i, j = self.convert_txy_to_tij(state)
        if self.obs_mask[t,i,j]: 
            return True
        else:
            return False
    ...
    def step(self, action):
        ...
        is_hitting_obs = None # doesnt (shouldnt) matter
        if not is_outbound:
            is_hitting_obs = self.is_hitting_obs()
        if has_reached_target:
            self.reward = self.xlim
            self.done = True
        elif is_outbound or is_hitting_obs:
            self.reward = -self.xlim
            self.done = True
            self.state =  old_s
